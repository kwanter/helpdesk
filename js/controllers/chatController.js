...
chat.controller('ChatCtrl', ['$rootScope','$scope','$route', '$http', '$q', '$state', 'DataService', 'TicketService', 
function ($rootScope,$scope,$route, $http, $q, $state, DataService, TicketService) {
    var ip = self.location.host;
    var url = 'http://'+ip+'/helpdesk_api/';
    $scope.id_ticket = DataService.get();
    $scope.urlListMessages = url + 'chat/' + $scope.id_ticket;
    $scope.urlListOnlines = url + 'chat/' + $scope.id_ticket;
    $scope.reset = reset;

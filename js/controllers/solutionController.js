...
solution.controller('SolutionCtrl', ['$scope', '$http','$q','$route','$state','DataService','TicketService','$localStorage', '$rootScope',
function ($scope, $http,$q,$route,$state,DataService,TicketService,$localStorage,$rootScope) {
    var ip = self.location.host;
    var url = 'http://'+ip+'/helpdesk_api/';
    $scope.id_ticket = DataService.get();
    $scope.urlListMessages = url + 'solution/' + $scope.id_ticket;
    //$scope.urlSaveMessage  = url + 'solution/' + $scope.id_ticket;
    $scope.urlListOnlines  = url + 'solution/' + $scope.id_ticket;
    $scope.reset = reset;
    ...
    function saveSolution(data) {
        TicketService.saveSolution(data)
            .then(
                reset,
    ...
    $scope.saveSolution = function () {
        ...
        var data = {
            'message': {
                ...
                'str_solution': $scope.me.solution,
                'user'        : $rootScope.user.id_user
            }
        }
        saveSolution(data);
    };
